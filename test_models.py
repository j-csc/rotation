...
def main():
    ...
    print("JASDHAKJDHAKSJDHAJKDAKJSDHKASDHKJAHSDahdkHKAJshdkajsdhkashd")
    ...
    rot_model = torch.load('./old_models/best_roteq_model.pth')
    ...
    # test_epoch_rot_unet = smp.utils.train.ValidEpoch(
    #     rot_model,
    #     loss=loss,
    #     metrics=metrics,
    #     device=device,
    ...
    aug_test_ds = ShipTestDataset('./data/test_set_rotation_aug/', transform=None)
    aug_test_loader = DataLoader(dataset=aug_test_ds, batch_size = 1, num_workers=1)

    test_ds = ShipTestDataset('./data/test_set/', transform=None)
